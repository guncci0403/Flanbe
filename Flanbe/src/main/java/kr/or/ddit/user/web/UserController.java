...
//			logger.debug("여기portfoliovo: {} ", portfoliovo);
	...
	public String detailpopol(int po_no, Model model, HttpSession session) {
		...
		PortfolioVo porfolio = userService.getPopolList(po_no);
		model.addAttribute("filesList", userService.selectfiles(po_no));
		model.addAttribute("detailpopol", porfolio);
		model.addAttribute("user", userService.selectUser(porfolio.getUser_id()));
		return "t/profile/detailPopol";
	}
	
	//대표 포트폴리오 설정,해제
	@RequestMapping(path="representpopol", method= {RequestMethod.POST})
	public String representpopol(String po_no, Model model) {
		logger.debug("po_no:{}",po_no);
		
		userService.representNPopol();   //모두 N으로 초기화작업
		...
		if(po_no != null) {
			...
			for (int i = 0; i < array.length; i++) {
				...
				if (pono != 0) {
					userService.representYPopol(pono);   //Y->N으로 변경작업
				}
			}
		}
		return "redirect:/user/mainpopol";
