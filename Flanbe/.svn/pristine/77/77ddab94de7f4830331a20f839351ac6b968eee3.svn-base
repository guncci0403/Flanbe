		...
		<div class="profile_frame">
			<div class="col-11 mt-4 pl-lg-0">
				<div class="card border  h-mail-list-section">
					<section>
     				<c:if test="${popolVo.po_no eq null}"><h3 class="p5-profile-head">포트폴리오 추가 </h3></c:if>
				    <c:if test="${popolVo.po_no ne null}"><h3 class="p5-profile-head">포트폴리오 수정 
				    	<a href="${cp }/user/detailpopol?po_no=${popolVo.po_no}" class="bg-primary float-right mr-3 py-1 px-1 rounded text-white"><!-- 뒤로가기 버튼 -->
                        <i class="fas h5 mr-2 fa-angle-left" style="margin: 6px;"></i></a></h3> </c:if>		     
					</section>
												...
												<div class="form-group row">
												  <label class="col-sm-2 col-form-label"><span class="popolspan">* &nbsp;</span>개발분야</label> 
													...
													<div class="form-group row">
														...
														<c:choose>
														<c:when test="${popolVo.po_no ne null}"> <!-- 포트폴리오 수정일 때-->
															<textarea class="form-control p5-portfolio-description" rows="10" id="p5-form-description" name="po_cont">${popolVo.po_cont}</textarea>
														</c:when>
														<c:otherwise> <!-- 포트폴리오 등록일 때-->
															<textarea class="form-control p5-portfolio-description"id="p5-form-description" name="po_cont" rows="10"></textarea>
														</c:otherwise>
														</c:choose>
														</div>
													</div>
